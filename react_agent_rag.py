...
def assistant_node(state: AgentState) -> Dict[str, Any]:
    """
    Nodo Assistant que RESPETA el reasoning previo.
    Extrae la acción del reasoning y la ejecuta.
    """
    llm = _lazy_components.get('llm')
    
    # Buscar el último reasoning en los mensajes
    last_reasoning = extract_last_reasoning(state["messages"])
    
    if last_reasoning:
        # El reasoning ya dicta qué hacer, solo ejecutar
        action = last_reasoning.get("action")
        parameters = last_reasoning.get("parameters")
        
        if action == "final_answer":
            # Generar respuesta final directamente
            return {"messages": [AIMessage(content=parameters.get("response", "Respuesta final generada."))]}
        # Si es una tool, el ToolNode la manejará
    else:
        # No hay reasoning, generar uno nuevo (fallback)
        print("⚠️ No se encontró reasoning. Invocando reasoning_node desde assistant.")
        return reasoning_node(state)
    
    # Preparar tools para binding
    ...
    # Invocar LLM con el contexto completo
...
# ========================================================================
# NUEVO NODO: REASONING EXPLÍCITO
# ========================================================================
import re

def reasoning_node(state: AgentState) -> Dict[str, Any]:
    """
    Forzar al LLM a EXPLICITAR su razonamiento antes de actuar.
    Este nodo es el CORAZÓN del patrón ReAct verdadero.
    """
    llm = _lazy_components.get('llm')

    if not llm:
        raise ValueError("LLM no inicializado en _lazy_components")

    # Contadores del estado
    attempt_count = state.get("attempt_count", 0)
    execution_success = state.get("execution_success", None)
    sql_query = state.get("sql_query", None)
    rag_attempt_count = state.get("rag_attempt_count", 0)

    # Si ya se hizo más de 3 intentos, forzar final_answer
    if attempt_count >= 3:
        print("🧠 MAX ATTEMPTS REACHED → Forzando final_answer")
        return {
            "messages": [AIMessage(content="ACTION: final_answer\nPARAMETERS: {\"response\": \"Respuesta parcial generada tras varios intentos.\"}")]
        }

    # Prompt obligatorio para estructurar el razonamiento
    reasoning_prompt = f"""
Eres un agente analítico que SIGUE ESTRICTAMENTE el patrón ReAct.

ANTES de usar cualquier herramienta, DEBES producir un razonamiento 
explícito con esta estructura OBLIGATORIA:

=== FORMATO OBLIGATORIO ===
REASONING: [Tu análisis detallado aquí]
- ¿Qué información necesito para responder?
- ¿Qué datos ya tengo del estado anterior?
- ¿Hay errores previos que debo considerar?

ACTION: [nombre_exacto_de_la_herramienta | final_answer]
- Opciones: execute_sql, get_kpi_sql, retrieve_documents, final_answer

PARAMETERS: {{json válido con los parámetros}}
- Ejemplo: {{"query": "SELECT...", "kpi_name": "ventas_por_sede"}}
- Para final_answer: {{"response": "tu respuesta al usuario"}}

CONTINGENCY: [Qué harás si esta acción falla]
- Ejemplo: "Si SQL falla, intentaré con KPI predefinido"
- Ejemplo: "Si no hay resultados, ampliaré el rango de fechas"

=== CONTEXTO DE EJECUCIÓN ===
- Intento actual: {attempt_count + 1}
- Última ejecución: {'✅ ÉXITO' if execution_success else '❌ FALLÓ' if execution_success is not None else '⏳ PENDIENTE'}
- Intentos RAG: {rag_attempt_count}/2
- Última query: {sql_query[:100] + '...' if sql_query else 'N/A'}

=== REGLAS CRÍTICAS ===
1. NUNCA uses una herramienta sin razonamiento previo
2. NUNCA omitas el formato obligatorio
3. Si es la 3ra iteración, DEBES dar final_answer aunque sea parcial
"""

    # Invocar LLM para generar razonamiento estructurado
    response = llm.invoke([
    ...
    # Validar formato
    if not validate_reasoning_format(response.content):
        print("⚠️ Formato inválido detectado. Solicitando corrección.")
        correction_prompt = """
❌ FORMATO INVÁLIDO

Tu razonamiento no sigue el formato obligatorio.
Debes incluir: REASONING, ACTION, PARAMETERS, CONTINGENCY

Reintenta con el formato correcto.
"""
...
def create_react_graph_real(llm, db) -> StateGraph:
    """Crea el grafo ReAct REAL con nodo de reasoning explícito"""
    ...
    # === AGREGAR NODOS ===
    workflow.add_node("reasoning", reasoning_node)      # ← NUEVO
    workflow.add_node("assistant", assistant_node)
    workflow.add_node("tools", tool_node)
    workflow.add_node("observer", observer_node)
    
    # === SET ENTRY POINT ===
    workflow.set_entry_point("reasoning")  # ← CAMBIADO: empieza con reasoning
    
    # === EDGES CONDICIONALES ===
    def route_from_reasoning(state):
        return "assistant"
    ...
    workflow.add_edge("observer", "reasoning")  # ← CAMBIADO: vuelve a reasoning, no assistant
    
    # Compilar
    compiled_graph = workflow.compile()
    compiled_graph.max_iterations = 15  # ← AUMENTADO de 5 a 15
...
def validate_reasoning_format(content: str) -> bool:
    """
    Validar que el reasoning tenga la estructura obligatoria.
    Returns True si es válido, False si necesita corrección.
    """
    required_sections = ["REASONING:", "ACTION:", "PARAMETERS:"]
    
    for section in required_sections:
        if section not in content.upper():
            print(f"⚠️ Falta sección: {section}")
            return False
    
    # Validar ACTION
    valid_actions = ["execute_sql", "get_kpi_sql", "retrieve_documents", "final_answer"]
    action_line = [line for line in content.split('\n') if 'ACTION:' in line.upper()]
    if action_line:
        action_value = action_line[0].split(':', 1)[1].strip().lower()
        if not any(valid_action in action_value for valid_action in valid_actions):
            print(f"⚠️ ACTION inválido: {action_value}")
            return False
    
    # Validar PARAMETERS como JSON
    param_line = [line for line in content.split('\n') if 'PARAMETERS:' in line.upper()]
    if param_line:
        try:
            param_json = param_line[0].split(':', 1)[1].strip()
            json.loads(param_json)
        except Exception as e:
            print(f"⚠️ PARAMETERS no es JSON válido: {e}")
            return False
...
def extract_last_reasoning(messages: List) -> Optional[Dict]:
    """
    Extraer el último reasoning válido de la lista de mensajes.
    Returns dict con action, parameters, contingency o None.
    """
    for msg in reversed(messages):
        if hasattr(msg, 'content') and isinstance(msg.content, str):
            content = msg.content
            
            reasoning_match = re.search(r'REASONING:(.*?)(?=ACTION:|$)', content, re.DOTALL | re.IGNORECASE)
            action_match = re.search(r'ACTION:\s*(\w+)', content, re.IGNORECASE)
            params_match = re.search(r'PARAMETERS:\s*({.*?})', content, re.DOTALL | re.IGNORECASE)
            ...
            if action_match and params_match:
                try:
                    ...
                    print(f"⚠️ Error parseando reasoning: {e}")
                    continue
